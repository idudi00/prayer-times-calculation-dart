...
class PrayerTimesSDK {
  ...
  double _calculateSunrise(double declination, double eqTime) {
    final hourAngle = Astronomical.calculateHourAngle(latitude, declination, -0.833);
    if (hourAngle.isNaN) return double.nan;

    final timeCorrection = eqTime + 4 * longitude - 60 * timezone;
    final sunrise = 12 - Astronomical.timeFromAngle(hourAngle) - timeCorrection / 60;
  ...
  double _calculateSunset(double declination, double eqTime) {
    ...
    final timeCorrection = eqTime + 4 * longitude - 60 * timezone;
    final sunset = 12 + Astronomical.timeFromAngle(hourAngle) - timeCorrection / 60;
  ...
  double _calculateFajr(double declination, double eqTime, double fajrAngle) {
    final hourAngle = Astronomical.calculateHourAngle(latitude, declination, -fajrAngle);
    if (hourAngle.isNaN) return double.nan;

    final timeCorrection = eqTime + 4 * longitude - 60 * timezone;
    final fajr = 12 - Astronomical.timeFromAngle(hourAngle) - timeCorrection / 60;
  ...
  double _calculateIsha(double declination, double eqTime, double ishaAngle) {
    final hourAngle = Astronomical.calculateHourAngle(latitude, declination, -ishaAngle);
    if (hourAngle.isNaN) return double.nan;

    final timeCorrection = eqTime + 4 * longitude - 60 * timezone;
    final isha = 12 + Astronomical.timeFromAngle(hourAngle) - timeCorrection / 60;
    return isha >= 0 ? isha : isha + 24;
  }

  /// Calculate Asr (afternoon) prayer time
  double _calculateAsr(double declination, double eqTime, AsrJurisdiction jurisdiction) {
    ...
    final hourAngle = Astronomical.calculateHourAngle(latitude, declination, 90 - altitude);
    if (hourAngle.isNaN) return double.nan;

    final timeCorrection = eqTime + 4 * longitude - 60 * timezone;
    final asr = 12 + Astronomical.timeFromAngle(hourAngle) - timeCorrection / 60;
